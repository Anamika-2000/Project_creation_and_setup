...
if ! which python3 >/dev/null 2>&1; then
    echo "${yellow}Python 3 is not installed. Installing Python 3...${reset}"
      yum install -y python3
    if [ $? -ne 0 ]; then
        echo "${red}Error: Failed to installPython 3${reset}"
        exit 1
    fi
    echo "${green}Python 3 has been installed${reset}"
else
    echo "${green}Python 3 is already installed${reset}"
fi
 echo "${colour}--------------------------------------------------------------------------------------------------------------------------------------------${reset}"
...
if [ "$name" = "main" -o "$name" = "MAIN" ]; then
 ...
 echo "${colour}--------------------------------------------------------------------------------------------------------------------------------------------${reset}"
echo "${green}Project set-up successfully for python project !${reset}"
echo "${green}You have installed git, vim, Python 3 :)${reset}"