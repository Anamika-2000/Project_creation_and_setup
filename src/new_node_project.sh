...
if which node >/dev/null 2>&1; then
    ...
    if [ $? -ne 0 ]; then
        echo "${red}Error: Failed to install Node.js${reset}"
        exit 1
    fi
...
npm init -y
if [ $? -ne 0 ]; then
    echo "${red}Error: Failed to initialize npm${reset}"
    exit 1
fi
...
if [ $? -ne 0 ]; then
 ...
 echo "${colour}--------------------------------------------------------------------------------------------------------------------------------------------${reset}"
 echo "${green}Congratulations! You have Git, Vim editor, Node.js, and your project folder set up. All done!${reset}"
